...
if ((${#staged_files[@]} == 0)); then
  echo "No staged *.c/*.h files; skipping clang-format/cppcheck."
  exit 0
fi
...
tmp_out="${TMPDIR:-/tmp}/cppcheck.$$.txt"
...
errors="$(rg -n ': error:' "$tmp_out" || true)"
warnings="$(rg -n ': warning:' "$tmp_out" || true)"
...
if [[ -n "$errors" || -n "$warnings" ]]; then
  ...
  echo
  echo "Full cppcheck output saved at: $tmp_out"
  exit 1
fi
