...
{
	atf_tc_set_md_var(
	    tc, "descr", "Test mport_mkdir creates a directory successfully with mode 755");
}
...
{
	atf_tc_set_md_var(
	    tc, "descr", "Test mport_mkdir returns MPORT_OK if directory already exists");
}
...
ATF_TC_CLEANUP(mport_mkdir_existing, tc)
{
	...
	ATF_TC_BODY(mport_check_answer_bool_invalid, tc)
	{
		...
		static void create_file(const char *filename, const void *data, size_t len)
		{
		...
		{
			atf_tc_set_md_var(
			    tc, "descr", "mport_is_elf_file returns true for an ELF file");
		}
		...
		{
			atf_tc_set_md_var(
			    tc, "descr", "mport_is_elf_file returns false for a plain text file");
		}
		...
		ATF_TC_HEAD(is_elf_file_false_small, tc)
		{
			atf_tc_set_md_var(tc, "descr",
			    "mport_is_elf_file returns false for a file smaller than ELF magic");
		}
		...
		{
			atf_tc_set_md_var(
			    tc, "descr", "mport_rmdir successfully removes an empty directory");
		}
		...
		{
			atf_tc_set_md_var(tc, "descr",
			    "mport_rmdir handles non-empty directory with ignore_nonempty=1");
		}
		...
		{
			atf_tc_set_md_var(tc, "descr",
			    "mport_rmdir fails on non-empty directory with ignore_nonempty=0");
		}
		...
		{
			atf_tc_set_md_var(tc, "descr",
			    "mport_rmdir handles non-existent directory with ignore_nonempty=1");
		}
